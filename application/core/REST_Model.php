...
class REST_Model extends CI_Model 
{
	...
	public function create($data, $fields = FALSE, $table = FALSE)
	{
		...
		// encode "data" or "address" if exists
	...
	public function get_by_id($id,$fields = FALSE,$table = FALSE)
	{
		...
		if ($this->exists($id))
		{
			...
			$row = $this->db->select($fields)->get_where($table, array('id' => $id))->row_array();
	...
	public function get_all($where = FALSE,$like = FALSE,$fields = FALSE,$page = 1,$limit = DEFAULT_QUERY_LIMIT,$sort_field = FALSE,$sort_order = 'asc',$table = FALSE)
	{
		...
		foreach($query->result_array() as $datum)
		{
			$data[] = $this->_format($datum);
		}
	 ...
	 */
	protected static function _format($data)
	{
		foreach($data as $key  => $value)
		{
			if ($key === 'data' || $key === 'address')
			{
				if (is_array($value))
                {
